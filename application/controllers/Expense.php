    ...
    public function index() {
        ...
        if ($this->session->userdata('sessionID') == "admin") {
            $query = $this->Md->query("SELECT *,expenses.id AS id  FROM expenses LEFT JOIN company ON company.id =  expenses.companyID");
             ...
             $query = $this->Md->query("SELECT *,user.name AS user,expenses.id AS id  FROM expenses LEFT JOIN company ON company.id =  expenses.companyID LEFT JOIN user ON user.id = expenses.userID WHERE expenses.companyID ='" . $this->session->userdata('companyID') . "'");
          }
        if ($query) {
            $data['clients'] = $query;
        } else {
